...
function simulate(){
  ...
  for(const item of units){
    ...
    for(let si=0;si<spaces.length;si++)for(const d of rotations){const s=spaces[si];if(d[0]<=s.l&&d[1]<=s.w&&d[2]<=s.h){const score=compactPlacementScore(s,d,placed,c);if(!best||score<best.score)best={si,d,score}}}
...
function shiftCargoInside(placed,c){if(!placed.length)return;const maxX=Math.max(...placed.map(p=>p.x+p.l)),shift=Math.max(0,c.l-maxX);placed.forEach(p=>p.x+=shift)}
function buildSecuringPlan(load){
  const dunnage=[],airbags=[],floorItems=load.placed.filter(p=>p.z===0),c=load.container;
  const doorGap=floorItems.length?Math.min(...floorItems.map(p=>p.x)):0,useDoorAirbag=doorGap>=300&&doorGap<=600;
  if(floorItems.length&&!useDoorAirbag){
  ...
  {
    const candidates=[];
    if(useDoorAirbag)load.placed.filter(p=>Math.abs(p.x-doorGap)<2).forEach(p=>{const width=Math.min(700,p.w*.65),height=Math.min(1200,p.h*.78);candidates.push({type:'airbag',zone:'door',x:0,y:p.y+(p.w-width)/2,z:p.z+Math.max(20,p.h*.11),l:p.x,w:width,h:height,location:`문쪽 간극${p.z>0?` · ${(p.z/1000).toFixed(1)}m 높이`:''}`,product:p.name})});
    ...
    candidates.filter(free).sort((a,b)=>b.z-a.z||(b.l*b.w*b.h-a.l*a.w*a.h)).forEach(q=>{const duplicate=airbags.some(a=>overlapRatio(a.x,a.x+a.l,q.x,q.x+q.l)>.55&&overlapRatio(a.y,a.y+a.w,q.y,q.y+q.w)>.55&&overlapRatio(a.z,a.z+a.h,q.z,q.z+q.h)>.35);if(!duplicate)airbags.push(q)});
    const zoneName={left:'좌측 벽 간극',right:'우측 벽 간극',back:'안쪽 벽 간극',door:'문쪽 간극'};
    let combined=true;
...
function testContainer(c,priority){
  ...
  for(const item of units){
    ...
    for(let si=0;si<spaces.length;si++)for(const d of allowedRotations(item)){const s=spaces[si];if(d[0]<=s.l&&d[1]<=s.w&&d[2]<=s.h){const score=compactPlacementScore(s,d,placed,c);if(!best||score<best.score)best={si,d,score}}}
...
function renderSecuringRecommendation(){const el=$('securingRecommendation'),plan=result.securing;if(!plan||(!plan.dunnage.length&&!plan.airbags.length)){el.hidden=true;el.innerHTML='';return}el.hidden=false;const items=[...plan.dunnage.slice(0,6).map((d,i)=>`<div><strong>문쪽 고정 부목 ${i+1} · ${d.location}</strong>${esc(d.product)} 하단 · X ${(d.x/1000).toFixed(2)}m · Y ${(d.y/1000).toFixed(2)}m</div>`),...plan.airbags.slice(0,8).map((a,i)=>`<div><strong>에어백 ${i+1} · ${a.location}</strong>X ${(a.x/1000).toFixed(2)}m · Y ${(a.y/1000).toFixed(2)}m · Z ${(a.z/1000).toFixed(2)}m</div>`)];el.innerHTML=`<h3>컨테이너 ${result.containerNumber} 화물 고정재 추천</h3><p>문쪽 마지막 화물 하단 부목 ${plan.dunnage.length}개 · 벽면 및 화물 사이 에어백 ${plan.airbags.length}개${plan.dunnage.length>6||plan.airbags.length>8?' · 대표 위치만 표시':''}. 화물은 안쪽 벽부터 배치하며, 문과 300~600mm가 남으면 에어백을, 더 큰 공간에는 마지막 화물 하단 고정 부목을 추천합니다. 실제 설치 전 바닥 못 고정 허용 여부와 제조사 지침을 확인하세요.</p><div class="securing-items">${items.join('')}</div>`}
...
function toggleSecuringVisibility(type){const isDunnage=type==='dunnage';if(isDunnage)showDunnage=!showDunnage;else showAirbags=!showAirbags;const button=$(isDunnage?'toggleDunnage':'toggleAirbags'),active=isDunnage?showDunnage:showAirbags;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active));draw()}
