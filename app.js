...
function buildSecuringPlan(load){
  ...
  if($('useAirbags').checked){
    ...
    candidates.filter(free).sort((a,b)=>b.z-a.z||(b.l*b.w*b.h-a.l*a.w*a.h)).forEach(q=>{const duplicate=airbags.some(a=>overlapRatio(a.x,a.x+a.l,q.x,q.x+q.l)>.55&&overlapRatio(a.y,a.y+a.w,q.y,q.y+q.w)>.55&&overlapRatio(a.z,a.z+a.h,q.z,q.z+q.h)>.35);if(!duplicate&&airbags.length<14)airbags.push(q)});
  }
...
function renderSecuringRecommendation(){const el=$('securingRecommendation'),plan=result.securing;if(!plan||(!plan.dunnage.length&&!plan.airbags.length)){el.hidden=true;el.innerHTML='';return}el.hidden=false;const items=[...plan.dunnage.slice(0,6).map((d,i)=>`<div><strong>문쪽 고정 부목 ${i+1} · ${d.location}</strong>${esc(d.product)} 하단 · X ${(d.x/1000).toFixed(2)}m · Y ${(d.y/1000).toFixed(2)}m</div>`),...plan.airbags.slice(0,8).map((a,i)=>`<div><strong>에어백 ${i+1} · ${a.location}</strong>X ${(a.x/1000).toFixed(2)}m · Y ${(a.y/1000).toFixed(2)}m · Z ${(a.z/1000).toFixed(2)}m</div>`)];el.innerHTML=`<h3>컨테이너 ${result.containerNumber} 화물 고정재 추천</h3><p>문쪽 하단 고정 부목 ${plan.dunnage.length}개 · 벽면 및 화물 사이 에어백 ${plan.airbags.length}개${plan.dunnage.length>6||plan.airbags.length>8?' · 대표 위치만 표시':''}. 부목은 문쪽 개방면에만 적용하고 좌·우·안쪽 벽면 간극은 에어백으로 고정합니다. 실제 설치 전 바닥 못 고정 허용 여부와 제조사 지침을 확인하세요.</p><div class="securing-items">${items.join('')}</div>`}
